...
const BottomNav = () => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 glass-nav border-t border-border/50 z-50 safe-area-pb">
      <div className="max-w-md mx-auto flex items-center justify-around py-2 px-2">
        {items.map((item) => {
          ...
          if (item.special) {
            return (
              <button key={item.label} onClick={() => {
                ...
                <div className="w-14 h-14 rounded-full gradient-success flex items-center justify-center shadow-lg glow-success transition-transform active:scale-95">
          ...
          return (
            <button key={item.label} onClick={() => navigate(item.path)} className="flex flex-col items-center py-1 transition-all duration-200 active:scale-95">
              ...
              <span className={`text-[10px] mt-0.5 transition-colors ${active ? 'text-primary font-semibold' : 'text-muted-foreground'}`}>{item.label}</span>
            </button>
          );
        })}
      </div>
